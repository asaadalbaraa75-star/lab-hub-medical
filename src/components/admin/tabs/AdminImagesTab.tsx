...
// دالة ضغط الصور من معرض الهاتف قبل الرفع لمنع البطء وتعليق الأزرار
...
export const AdminImagesTab: React.FC<Props> = ({ currentUser }) => {
  ...
  // المزامنة الفورية الشاملة لجميع الصور والدروس
  useEffect(() => {
    setIsLoading(true);
    let isMounted = true;

    // 1. مزامنة الصور المركزية
    const unsubSlides = onSnapshot(
      ...
      (err) => {
        console.error('Firestore slides sync error:', err);
        setIsLoading(false);
      }
    );
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith('image/')) {
      setFeedback({ type: 'error', message: 'يرجى اختيار صورة صالحة (PNG, JPG, WebP).' });
      return;
    }
  ...
  const handleSaveImageForm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ضغط ورفع الصورة إلى Firebase Storage
      if (uploadMode === 'file' && selectedFile) {
        const compressedBlob = await compressImageFile(selectedFile);
        const storageRef = ref(storage, `slides/${Date.now()}_${selectedFile.name.replace(/[^a-zA-Z0-9.]/g, '_')}`);
        const uploadResult = await uploadBytes(storageRef, compressedBlob);
        finalImageUrl = await getDownloadURL(uploadResult.ref);
      }

      if (!finalImageUrl) {
        setFeedback({ type: 'error', message: 'يرجى اختيار صورة من المعرض أو وضع رابط مباشر.' });
        setIsSaving(false);
        return;
      }

      const selectedPractical = practicals.find((p) => p.id === formLessonId);
      ...
      const imageData = {
        ...
        lessonTitle: selectedPractical?.title || null,
        stainOrView: formStainOrView.trim(),
        magnification: formMagnification.trim(),
        url: finalImageUrl,
      ...
      if (editingImage) {
        // استبدال وتعديل
        await updateDoc(doc(db, 'slides', editingImage.id), imageData);
        setFeedback({ type: 'success', message: 'تم استبدال الصورة بنجاح وتحديثها لجميع الطلاب والمستخدمين!' });
      } else {
        // إضافة صورة جديدة
        ...
        setFeedback({ type: 'success', message: 'تمت إضافة الصورة بنجاح وحفظها في قاعدة البيانات المشتركة.' });
      }

      setIsEditModalOpen(false);
    } catch (err: any) {
      setFeedback({ type: 'error', message: 'حدث خطأ أثناء الحفظ: ' + err.message });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDeleteImage = async (id: string) => {
    ...
    try {
      ...
      setFeedback({ type: 'success', message: 'تم حذف الصورة بنجاح من قاعدة البيانات المشتركة.' });
  ...
  const handleSaveAssign = async () => {
    if (!imageToAssign || !targetLessonId) return;

    setIsSaving(true);
    try {
      const targetPrac = practicals.find((p) => p.id === targetLessonId);
      await updateDoc(doc(db, 'slides', imageToAssign.id), {
        lessonId: targetLessonId,
        lessonTitle: targetPrac?.title || targetLessonId,
        caption: targetCaption.trim(),
        updatedAt: new Date().toISOString()
      });

      setFeedback({ type: 'success', message: `تم تعيين الصورة للدرس "${targetPrac?.title}" بنجاح.` });
      setIsAssignModalOpen(false);
    } catch (e: any) {
      setFeedback({ type: 'error', message: 'خطأ أثناء التعيين: ' + e.message });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  // تجميع الصور وعرضها شجرياً حسب الدرس والوحدة
    ...
    const list = images.filter((img) => {
      if (searchQuery.trim()) {
        ...
        const matches =
          ...
          img.lessonTitle?.toLowerCase().includes(q);
        if (!matches) return false;
      }
      if (selectedSubject !== 'all' && img.subject !== selectedSubject) return false;
      return true;
    });

    const groups: { [key: string]: { lessonTitle: string; subject: string; items: ManagedImage[] } } = {};
    list.forEach((img) => {
      const key = img.lessonTitle || 'صور عامة وغير معينة لدرس';
      if (!groups[key]) {
        groups[key] = { lessonTitle: key, subject: img.subject, items: [] };
      }
    ...
    <div className="space-y-6 pb-12" dir="rtl">
      ...
      <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-black text-slate-900">إدارة و تحكم جميع صور المنصة (All Image Control)</h1>
            <p className="text-xs text-slate-500">إضافة، استبدال، وتخصيص صور السلايدات والدروس للجميع بشكل فورى</p>
          </div>
        </div>
        ...
        {canManage && (
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold shadow-md transition-all shrink-0"
          >
            <Plus className="w-4 h-4" />
            <span>إضافة صورة من الهاتف</span>
          </button>
        )}
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm flex flex-col md:flex-row items-center justify-between gap-3">
        <div className="relative flex-1 w-full">
          ...
          <input
            ...
            placeholder="ابحث باسم الصورة، الشرح، أو اسم الدرس..."
      ...
      {/* العرض الشجري المنظم للصور حسب الدروس */}
      {isLoading ? (
        <div className="py-20 text-center bg-white rounded-3xl border border-slate-200">
          <RefreshCw className="w-8 h-8 text-rose-500 animate-spin mx-auto mb-2" />
          <p className="text-xs text-slate-500 font-bold">جاري جلب جميع الصور من السيرفر المركزي...</p>
        </div>
      ) : Object.keys(groupedImages).length === 0 ? (
        <div className="py-16 text-center bg-white rounded-3xl border border-slate-200 space-y-2">
          <ImageIcon className="w-10 h-10 text-slate-300 mx-auto" />
          <p className="text-xs text-slate-500 font-bold">لا توجد صور مضافة حتى الآن. يمكنكِ إضافة أول صورة من الهاتف بالأعلى.</p>
        </div>
      ) : (
        <div className="space-y-6">
          {Object.entries(groupedImages).map(([lessonName, group]) => (
            <div key={lessonName} className="bg-white rounded-3xl border border-slate-200 p-5 shadow-sm space-y-4">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                <div className="flex items-center gap-2">
                  <FolderTree className="w-5 h-5 text-rose-600" />
                  <h3 className="text-sm font-black text-slate-900">{lessonName}</h3>
                  <span className="text-[10px] bg-slate-100 px-2 py-0.5 rounded-full font-bold text-slate-600">
                    {group.items.length} صورة
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {group.items.map((img) => (
                  <div key={img.id} className="bg-slate-50 rounded-2xl border border-slate-200 overflow-hidden flex flex-col justify-between hover:shadow-md transition-all">
                    <div className="relative aspect-[4/3] bg-slate-900 flex items-center justify-center">
                      ...
                      <button
                        ...
                        className="absolute top-2 left-2 p-1.5 bg-black/60 text-white rounded-lg"
                      >
                    ...
                    <div className="p-3 space-y-2">
                      <div>
                        ...
                        {img.caption && <p className="text-[11px] text-slate-500 line-clamp-2 mt-0.5">{img.caption}</p>}
                      </div>

                      <div className="pt-2 border-t border-slate-200/60 flex items-center justify-between text-[10px]">
                        <button type="button" onClick={() => handleCopyUrl(img.url, img.id)} className="text-slate-500 hover:text-slate-800 flex items-center gap-1">
                        ...
                        {canManage && (
                          <div className="flex items-center gap-1">
                            <button
                              ...
                              className="px-2 py-0.5 rounded bg-indigo-50 text-indigo-700 font-bold border border-indigo-200"
                            >
                              تعيين
                            </button>
                            <button
                              ...
                              className="px-2 py-0.5 rounded bg-slate-900 text-white font-bold"
                            >
                              استبدال
                            </button>
                            <button
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto" dir="rtl">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-slate-200 space-y-4 my-8">
            <div className="flex items-center justify-between border-b pb-3">
              <h3 className="text-sm font-black text-slate-900">{editingImage ? 'استبدال وتعديل الصورة' : 'إضافة صورة جديدة من الهاتف'}</h3>
              <button type="button" onClick={() => setIsEditModalOpen(false)}>
                <X className="w-5 h-5 text-slate-400" />
              </button>
            </div>

            <form onSubmit={handleSaveImageForm} className="space-y-3">
              <div className="flex rounded-xl bg-slate-100 p-1 text-xs font-bold">
                <button
                  type="button"
                  onClick={() => setUploadMode('file')}
                  className={`flex-1 py-1.5 rounded-lg ${uploadMode === 'file' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-600'}`}
                >
                  اختيار من المعرض
                </button>
                <button
                  ...
                  className={`flex-1 py-1.5 rounded-lg ${uploadMode === 'url' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-600'}`}
                >
                  رابط صورة مباشر
                </button>
              </div>
              ...
              {uploadMode === 'file' ? (
                <div>
                  ...
                  <div
                    ...
                    className="border-2 border-dashed border-slate-300 hover:border-rose-500 rounded-2xl p-5 text-center cursor-pointer bg-slate-50"
                  >
                    <Upload className="w-7 h-7 text-slate-400 mx-auto mb-1" />
                    <p className="text-xs font-bold text-slate-700">اضغطي هنا لاختيار صورة من الاستوديو</p>
                    {selectedFile && <p className="text-[11px] text-emerald-600 font-bold mt-1">المحدد: {selectedFile.name}</p>}
                  </div>
                </div>
              ) : (
                <input
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    placeholder="مثال: Anterior View"
                    className="w-full px-3 py-2 text-xs border rounded-xl bg-slate-50"
                  />
                </div>
                <div>
              ...
              <div>
                <label className="block text-xs font-bold mb-1">تعيين لدرس محدد</label>
                <select
                  value={formLessonId}
                  onChange={(e) => setFormLessonId(e.target.value)}
                  className="w-full px-3 py-2 text-xs border rounded-xl bg-slate-50"
                >
                  <option value="">-- صور عامة بدون تعيين لدرس --</option>
                  {availableLessonsForForm.map((p) => (
              ...
              <div>
                <label className="block text-xs font-bold mb-1">الشرح والتوضيح (Caption)</label>
                <textarea
                  rows={2}
                  value={formCaption}
                  onChange={(e) => setFormCaption(e.target.value)}
                  placeholder="ملاحظات سريرية أو تشريحية..."
                  className="w-full px-3 py-2 text-xs border rounded-xl bg-slate-50 resize-none"
                />
              </div>

              <div className="pt-3 border-t flex justify-end gap-2">
                <button type="button" onClick={() => setIsEditModalOpen(false)} className="px-4 py-2 text-xs font-bold text-slate-600">
                  إلغاء
                </button>
                <button
                  type="submit"
                  disabled={isSaving}
                  className="px-5 py-2 text-xs font-bold bg-rose-600 hover:bg-rose-700 text-white rounded-xl flex items-center gap-2"
                >
                  {isSaving && <RefreshCw className="w-3.5 h-3.5 animate-spin" />}
                  <span>{editingImage ? 'حفظ وتأكيد الاستبدال' : 'حفظ الصورة في المنصة'}</span>
                </button>
              </div>
            </form>
          </div>
      ...
      {isAssignModalOpen && imageToAssign && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" dir="rtl">
          <div className="bg-white rounded-3xl max-w-sm w-full p-6 space-y-4">
            <h3 className="text-xs font-black text-slate-900">تعيين الصورة لدرس عملي</h3>
            ...
            <div className="flex justify-end gap-2 pt-2">
              <button type="button" onClick={() => setIsAssignModalOpen(false)} className="px-3 py-1.5 text-xs font-bold">إلغاء</button>
              <button type="button" disabled={isSaving || !targetLessonId} onClick={handleSaveAssign} className="px-4 py-1.5 bg-indigo-600 text-white rounded-xl text-xs font-bold">
                تأكيد التعيين
              </button>
            </div>
          </div>
      ...
      {deleteConfirmId && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" dir="rtl">
          <div className="bg-white rounded-3xl max-w-xs w-full p-5 text-center space-y-3">
            ...
            <p className="text-xs font-bold">تأكيد حذف الصورة نهائياً؟</p>
            <div className="flex justify-center gap-2">
              <button type="button" onClick={() => setDeleteConfirmId(null)} className="px-3 py-1 text-xs font-bold">تراجع</button>
              <button type="button" disabled={isSaving} onClick={() => handleDeleteImage(deleteConfirmId)} className="px-4 py-1 bg-rose-600 text-white rounded-xl text-xs font-bold">
      ...
      {/* معاينة مكبرة للصور */}
      {previewModalUrl && (
        <div className="fixed inset-0 z-50 bg-black/90 flex flex-col items-center justify-center p-4" onClick={() => setPreviewModalUrl(null)}>
          <img src={previewModalUrl.url} alt={previewModalUrl.title} className="max-w-full max-h-[80vh] object-contain rounded-2xl" />
          <p className="text-white text-xs mt-2 font-bold">{previewModalUrl.title}</p>
        </div>
      )}
    </div>
  );
...
export const AdminImagesTab: React.FC<Props> = ({ currentUser }) => {
  ...
  // المزامنة الفورية السريعة مع Firestore
  useEffect(() => {
    setIsLoading(true);
    let isMounted = true;
    ...
    const unsubSlides = onSnapshot(
      ...
      (snap) => {
        if (!isMounted) return;
        const list: ManagedImage[] = [];
        snap.forEach((d) => list.push({ id: d.id, ...d.data() } as ManagedImage));
        setImages(list);
        setIsLoading(false);
      },
      (err) => {
  ...
  const handleSaveImageForm = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formTitle.trim()) {
      setFeedback({ type: 'error', message: 'يرجى إدخال عنوان الصورة.' });
      return;
    }
    ...
    try {
      ...
      if (!finalImageUrl) {
        setFeedback({ type: 'error', message: 'يرجى اختيار صورة أو إدخال رابط مباشر.' });
        setIsSaving(false);
        return;
      }
      ...
      if (editingImage) {
        await updateDoc(doc(db, 'slides', editingImage.id), imageData);
        setFeedback({ type: 'success', message: 'تم استبدال الصورة بنجاح!' });
      } else {
        await addDoc(collection(db, 'slides'), {
          ...imageData,
          createdAt: serverTimestamp()
        });
        setFeedback({ type: 'success', message: 'تمت إضافة الصورة وحفظها بنجاح!' });
      }
  ...
  const groupedImages = useMemo(() => {
    const list = images.filter((img) => {
      if (searchQuery.trim()) {
        const q = searchQuery.toLowerCase();
        if (!img.title?.toLowerCase().includes(q) && !img.lessonTitle?.toLowerCase().includes(q)) return false;
      }
      if (selectedSubject !== 'all' && img.subject !== selectedSubject) return false;
      return true;
    });
    ...
    <div className="space-y-6 pb-12" dir="rtl">
      <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-black text-slate-900">إدارة صور الدروس السريعة (Image Management)</h1>
            <p className="text-xs text-slate-500">إضافة واستبدال وحفظ الصور في السيرفر المركزي لجميع الأجهزة</p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl border p-4 shadow-sm flex flex-col md:flex-row items-center justify-between gap-3">
        <div className="relative flex-1 w-full">
          ...
          <input
            ...
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="ابحث بالاسم أو اسم الدرس..."
      ...
      {isLoading ? (
        <div className="py-20 text-center bg-white rounded-3xl border"><RefreshCw className="w-8 h-8 text-rose-500 animate-spin mx-auto mb-2" /><p className="text-xs text-slate-500 font-bold">جاري تحميل الصور...</p></div>
      ) : Object.keys(groupedImages).length === 0 ? (
        <div className="py-16 text-center bg-white rounded-3xl border"><p className="text-xs text-slate-500 font-bold">لا توجد صور مضافة بعد.</p></div>
      ) : (
        <div className="space-y-6">
          {Object.entries(groupedImages).map(([lessonName, group]) => (
            <div key={lessonName} className="bg-white rounded-3xl border p-5 shadow-sm space-y-4">
              <div className="flex items-center justify-between border-b pb-2">
                <span className="font-black text-xs text-slate-900 flex items-center gap-2"><FolderTree className="w-4 h-4 text-rose-600" /> {lessonName}</span>
                <span className="text-[10px] bg-slate-100 px-2 py-0.5 rounded-full font-bold text-slate-600">{group.items.length} صورة</span>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {group.items.map((img) => (
                  <div key={img.id} className="bg-slate-50 rounded-2xl border overflow-hidden flex flex-col justify-between">
                    <div className="relative aspect-[4/3] bg-slate-900">
                      <img src={img.url} alt={img.title} className="w-full h-full object-cover" />
                      <button type="button" onClick={() => setPreviewModalUrl({ url: img.url, title: img.title })} className="absolute top-2 left-2 p-1 bg-black/60 text-white rounded-md"><Maximize2 className="w-3 h-3" /></button>
                    </div>

                    <div className="p-3 space-y-2">
                      <h4 className="font-bold text-xs line-clamp-1">{img.title}</h4>
                      <div className="flex items-center justify-between pt-2 border-t text-[10px]">
                        <button type="button" onClick={() => handleCopyUrl(img.url, img.id)} className="text-slate-500 flex items-center gap-1">{copiedId === img.id ? <Check className="w-3 h-3 text-emerald-600" /> : <Copy className="w-3 h-3" />} نسخ</button>
                        {canManage && (
                          <div className="flex gap-1">
                            <button type="button" onClick={() => handleOpenEditModal(img)} className="px-2 py-0.5 bg-slate-900 text-white rounded font-bold">استبدال</button>
                            <button type="button" onClick={() => setDeleteConfirmId(img.id)} className="p-1 text-rose-600"><Trash2 className="w-3.5 h-3.5" /></button>
                          </div>
                        )}
                      </div>
                    </div>
      ...
      {isEditModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto" dir="rtl">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 space-y-4 my-8">
            <h3 className="font-black text-sm">{editingImage ? 'استبدال الصورة' : 'إضافة صورة من الهاتف'}</h3>
            <form onSubmit={handleSaveImageForm} className="space-y-3">
              <div className="flex rounded-xl bg-slate-100 p-1 text-xs font-bold">
                <button type="button" onClick={() => setUploadMode('file')} className={`flex-1 py-1 rounded-lg ${uploadMode === 'file' ? 'bg-white shadow' : ''}`}>من الاستوديو</button>
                <button type="button" onClick={() => setUploadMode('url')} className={`flex-1 py-1 rounded-lg ${uploadMode === 'url' ? 'bg-white shadow' : ''}`}>رابط مباشر</button>
              </div>

              {uploadMode === 'file' ? (
                <div>
                  <input type="file" ref={fileInputRef} onChange={handleFileChange} accept="image/*" className="hidden" />
                  <div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed p-4 rounded-xl text-center cursor-pointer bg-slate-50">
                    <Upload className="w-6 h-6 text-slate-400 mx-auto" />
                    <p className="text-xs font-bold mt-1">اضغطي لاختيار صورة من الهاتف</p>
                  </div>
                </div>
              ) : (
                <input type="url" value={formImage} onChange={(e) => setFormImage(e.target.value)} placeholder="https://..." className="w-full text-xs p-2 border rounded-xl" />
              )}

              <input type="text" required value={formTitle} onChange={(e) => setFormTitle(e.target.value)} placeholder="عنوان الصورة *" className="w-full text-xs p-2 border rounded-xl" />
              <select value={formSubject} onChange={(e) => { setFormSubject(e.target.value as any); setFormLessonId(''); }} className="w-full text-xs p-2 border rounded-xl font-bold">
                <option value="anatomy">Anatomy</option>
                <option value="histology">Histology</option>
                <option value="biochemistry">Biochemistry</option>
              </select>

              <select value={formLessonId} onChange={(e) => setFormLessonId(e.target.value)} className="w-full text-xs p-2 border rounded-xl">
                <option value="">-- اختاري الدرس المرتبط --</option>
                {availableLessonsForForm.map((p) => (<option key={p.id} value={p.id}>{p.title}</option>))}
              </select>

              <div className="flex justify-end gap-2 pt-2">
                <button type="button" onClick={() => setIsEditModalOpen(false)} className="px-3 py-1.5 text-xs font-bold">إلغاء</button>
                <button type="submit" disabled={isSaving} className="px-4 py-1.5 bg-rose-600 text-white rounded-xl text-xs font-bold flex items-center gap-1">
                  {isSaving && <RefreshCw className="w-3 h-3 animate-spin" />} حفظ الصورة
                </button>
              </div>
            </form>
          </div>
      ...
      {deleteConfirmId && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" dir="rtl">
          <div className="bg-white rounded-3xl max-w-xs w-full p-5 text-center space-y-3">
            ...
            <div className="flex justify-center gap-2">
              <button type="button" onClick={() => setDeleteConfirmId(null)} className="px-3 py-1 text-xs font-bold">تراجع</button>
              <button type="button" onClick={() => handleDeleteImage(deleteConfirmId)} className="px-4 py-1 bg-rose-600 text-white rounded-xl text-xs font-bold">حذف</button>
            </div>
          </div>
        </div>
      )}
